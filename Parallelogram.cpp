...
ostream& operator<< (ostream& out, Parallelogram pr) { //перегруженный оператор вывода
    if (pr.colorIndex < 0 || pr.colorIndex > 4) throw exception("¬ыбранный индекс цвета вне диапазона!");
    out << "ѕараллелограмм, построенный на векторах:" << endl << pr.a << pr.b << "÷вет фигуры: " << Figure::colors[pr.colorIndex];
    return out;
}